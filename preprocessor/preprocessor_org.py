...
class Preprocessor:
    ...
    def build_from_path(self):
        ...
        # Write metadata
        with open(os.path.join(self.out_dir, "V1-train.txt"), "w", encoding="utf-8") as f:
            for m in out[self.val_size :]:
                f.write(m + "\n")
        with open(os.path.join(self.out_dir, "V1-val.txt"), "w", encoding="utf-8") as f:
            for m in out[: self.val_size]:
                f.write(m + "\n")
    ...
    def process_utterance(self, speaker, chapter, basename):
        ...
        ###### xvector ######
        # if sr != 16000:
        #     # signal = torchaudio.functional.resample(wav, sr, 16000)
        #     signal = librosa.resample(wav, orig_sr=sr, target_sr=16000)
        #     signal = torch.from_numpy(signal)
        #     # print(signal)
        # xvect_embeddings = classifier_Xvector.encode_batch(signal).squeeze(0) # shape: 1, 1, 192 --> 1, 192
        ...
        # ###### ecapa ######
        # ecapa_embeddings = classifier_Ecapa.encode_batch(signal).squeeze(0) # shape: 1, 1, 192 --> 1, 192
